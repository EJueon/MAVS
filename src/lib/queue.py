...
class Seed(object):
    ...
    def __init__(self, cl, coverage, root_seed, parent, metadata, ground_truth, sampleNo=0, generation=0, l0_ref=0, linf_ref=0, data=None, tid=[-1]):
        """Inits the object.
        ...
        self.data = data
        self.label = self.ground_truth
        try:
            self.prediction = metadata[0]
        except:
            self.prediction = metadata
...
class FuzzQueue(object):
    ...
    def log(self):
        ...
        current_time = time.time()
        tf.logging.info(
                "Metrics %s | corpus_size %s | crashes_size %s | mutations_per_second: %s | total_exces %s | last new reg: %s | last new adv %s | diversity %d | coverage: %s -> %s%%",
                self.criteria,
                queue_len,
                self.uniq_crashes,
                round(float(self.mutations_processed)/(current_time - self.start_time), 2),
                ...
                np.sum([len(self.errorCategories[sample]) for sample in self.errorCategories.keys()]),
                self.dry_run_cov,
                coverage
            )
