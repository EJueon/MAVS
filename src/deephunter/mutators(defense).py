...
class Mutators():
    ...
    def image_brightness(img, params):
        beta = params
        beta = np.tile([beta], np.shape(img))
        new_img = img + beta
    ...
    def image_noise(img, params):
        if params == 1:  # Gaussian-distributed additive noise.
            ...
            gauss = np.round(gauss.reshape(row, col, ch))
            ...
            mean = 0
            #var = 0.25
            sigma = 0.1#var ** 0.5 / ch
            gauss = np.random.normal(mean, sigma, (row, col, ch))
            gauss = gauss.reshape(row, col, ch)
            noisy = np.round(np.clip(img + img * gauss, 0, 255))
            return noisy.astype(np.uint8)
        ...
        for ii in range(try_num):
            ...
            if cl == 0:  # 0: can choose class A and B
                ...
                if tid in Mutators.classA:
                    ...
                    l2_ref = np.sum(np.square(sub)) ** 0.5
                    if l2_ref <= l2:#l0_ref <= l0 or linf_ref <= l_infinity:
            ...
            if cl == 1: # 0: can choose class A
                ...
                param = random.sample(params, 1)[0]
                img_new = transformation(copy.deepcopy(img), param)
                img_new2 = np.clip(img_new, 0, 255)
                sub = ref_img - img_new
